...
use Drupal\vercel_ai_sdk_mock\MockToolCall;
...
final class MockProvider extends AiProviderClientBase implements ChatInterface {
  ...
  public function chat(array|string|ChatInput $input, string $model_id, array $tags = []): ChatOutput {
    ...
    // A round that asks for tools has nothing to stream, and the script says
    // to ask once: the round after it already carries the results, which is
    // what tells the two apart without the provider keeping a counter.
    if ($answer->tools && !$this->toolsHaveRun($input)) {
      $message = new ChatMessage('assistant', '');
      $message->setTools(array_map(
        static fn (MockToolCall $call): ToolsFunctionOutput => $call->toToolsFunctionOutput(),
        $answer->tools,
      ));
  ...
  /**
   * Whether this turn already holds a tool result.
   */
  private function toolsHaveRun(array|string|ChatInput $input): bool {
    if (!$input instanceof ChatInput) {
      return FALSE;
    }
    foreach ($input->getMessages() as $message) {
      if ($message instanceof ChatMessage && $message->getRole() === 'tool') {
        return TRUE;
      }
    }
    return FALSE;
