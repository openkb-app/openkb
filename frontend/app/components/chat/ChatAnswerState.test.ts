...
describe('ChatAnswerState', () => {
  ...
  it('marks an answer that cited nothing as not from the knowledge base', () => {
    ...
    expect(root.querySelector('[data-test="answer-ungrounded"]')?.textContent)
      .toContain('Not from your knowledge base')
  ...
  it('names the block and the badge, and adds no second live region', () => {
    const root = mount({ grounding: grounding('ungrounded', 0), cited: 0, query: 'sourdough' })
    const note = root.querySelector('[data-test="answer-searched-note"]')
    ...
    expect(root.querySelector('[data-test="answer-ungrounded"]')?.getAttribute('role')).toBe('note')
