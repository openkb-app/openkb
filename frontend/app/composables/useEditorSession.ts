...
import { recordCiteInsert } from '~/editor/cite-insert'
...
export async function useEditorSession(
  ...
  /**
   * The citation picker, opened on the end of this block.
   *
   * The same `[[` and the same popover a link uses; what differs is only the
   * node the pick becomes, which `recordCiteInsert` says. Deferred until the
   * menu is gone for the same reason `insertBlockBelow` is: a closing dropdown
  ...
  function citeFromBlockMenu() {
    runAfterMenuClose(() => {
      ...
      const block = selectedBlock()
      if (!editor || !block) return
      // Inside the block, after its last child: a citation is inline, and the
      // position after the block is not.
      const at = block.pos + block.node.nodeSize - 1
      recordCiteInsert(at)
