...
final class VercelAiSdkProcessor extends ChatProcessorBase implements ContainerFactoryPluginInterface {
  ...
  private function readTurn(mixed $normalized, array &$calls): iterable {
    if ($normalized instanceof StreamedChatMessageIteratorInterface) {
      foreach ($normalized as $chunk) {
        foreach ($chunk->getTools() ?? [] as $call) {
          $calls[] = $call;
        }
   ...
   *
   * Each call is reported as it happens, so the drawer shows what the
   * assistant did rather than a pause. The results are appended as the
   * assistant's tool-call message plus one `tool` message per call, the shape
   * providers expect a tool round in.
   *
  ...
  private function runTools(array $calls, array &$chatMessages): \Generator {
    ...
    foreach ($calls as $call) {
      ...
      yield [
        ...
        'toolCallId' => $call->getToolId(),
        'toolName' => $call->getName(),
        'input' => $arguments ?: (object) [],
      ];

      $result = $this->tools->call($call->getName(), $arguments, (array) $this->configuration['tools']);
      ...
      $toolTurn->setToolsId($call->getToolId());
