...
/** Whether the summary is what the pane is showing. The chip is that state's
 *  switch. */
const summaryInPane = computed(() => paneContent.value === 'summary' && !paneHidden.value)

/** A chat on screen covers the summary, so the chip has nothing to switch. A
 *  chat behind a put-away pane is not on screen and does not block it. */
const summaryBlocked = computed(() => chatOpen.value && !paneHidden.value)

/** Why the chip does nothing — as a tooltip, and as the chip's description. */
const SUMMARY_BLOCKED_REASON = 'Close the chat to see the AI summary'

function toggleSummary() {
  if (summaryBlocked.value) return
  if (summaryInPane.value) hidePane()
  else {
    // A chat still open behind the put-away pane would take it back. Putting it
    // away keeps its conversation; the launcher is the way back to it.
...
const summaryWanted = computed(() => summaryInPane.value && paneWide.value)
  ...
  <ChromePageBody :pane="routeQuery.trim() ? 'summary' : undefined">
    <main id="main-content" tabindex="-1" class="mx-auto w-full max-w-[1100px] px-4 py-6 focus:outline-none sm:px-6 sm:py-8">
      ...
      <div class="mb-5 flex flex-wrap items-center gap-1.5">
        ...
        <!-- The only chip that is a control rather than a filter: it switches
             the pane between the summary and what was there before. -->
        <button
          type="button"
          data-testid="summary-chip"
          :aria-pressed="summaryInPane"
          :aria-disabled="summaryBlocked ? 'true' : undefined"
          :aria-describedby="summaryBlocked ? 'summary-chip-reason' : undefined"
          :title="summaryBlocked ? SUMMARY_BLOCKED_REASON : undefined"
          class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[12px] font-medium transition aria-disabled:cursor-default aria-disabled:opacity-50 not-aria-disabled:hover:bg-elevated"
          :class="summaryInPane
            ? 'border-(--okb-agent) bg-elevated text-(--okb-agent-text)'
        ...
        <span v-if="summaryBlocked" id="summary-chip-reason" class="sr-only">{{ SUMMARY_BLOCKED_REASON }}</span>
      </div>
