...
interface SpaceResource {
  ...
  attributes: {
    ...
    field_agent_review?: boolean | null
    outline?: string | null
  }
...
function toSummary(space: SpaceResource): SpaceSummary {
  return {
    ...
    // Unset reads as on, the same strict default Drupal applies.
    agentReview: space.attributes.field_agent_review ?? true,
...
const SPACE_FIELDS = ['label', 'path', 'description', 'drupal_internal__id', 'read_access', 'field_moderation', 'field_agent_review', 'managers', 'members', 'viewers', 'outline']
 ...
 * taken, and puts the creator on the managers roster. Every policy flag has a
 * field default, so each is sent only when the surface set it. The slug comes
 * back off the saved space — its path alias — and is what to navigate to; the
 * space page loads its own roster.
 */
export async function createSpace(event: H3Event, space: NewSpace): Promise<{ id: string, slug: string }> {
  ...
  if (space.agentReview !== undefined) attributes.field_agent_review = space.agentReview
...
export function spacePatchBody(uuid: string, settings: SpaceSettings) {
  ...
  // the policy flags and must reach Drupal.
  if (settings.moderation !== undefined) attributes.field_moderation = settings.moderation
  if (settings.agentReview !== undefined) attributes.field_agent_review = settings.agentReview
