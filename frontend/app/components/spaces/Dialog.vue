...
import { READ_ACCESS_OPTIONS, MODERATION_OPTIONS } from '~/utils/space-policy'
...
const opened = ref({ description: '', readAccess: 'members_only' as KbSpaceReadAccess, moderation: true })
...
watch(open, (isOpen) => {
  ...
  // defaults, members-only + moderated.
  name.value = ''
  ...
  opened.value = { description: description.value.trim(), readAccess: readAccess.value, moderation: moderation.value }
})
