...
export const comarkHandlers: Record<string, EditorHandler> = {
  ...
  // On a selection the `[[` takes the selected words' place and they come back
  // as the link's label (editor/doc-link-selection.ts); on a caret the link
  // carries the target's own title.
  docLink: {
    canExecute: editor => editor.can().insertContent('[['),
    execute: (editor) => {
      const { from, to } = editor.state.selection
      if (from === to) return editor.chain().focus().setTextSelection(to).insertContent('[[')
      recordLinkSelection(editor.state.doc.textBetween(from, to, ' '), from)
...
export const insertBlockItems = [
  ...
  { kind: 'callout', type: 'info', label: 'Callout (info)', icon: 'i-lucide-info', description: 'Blue info box' },
  { kind: 'callout', type: 'warning', label: 'Callout (warning)', icon: 'i-lucide-triangle-alert', description: 'Yellow warning box' },
  { kind: 'callout', type: 'success', label: 'Callout (success)', icon: 'i-lucide-circle-check', description: 'Green success box' },
  { kind: 'callout', type: 'danger', label: 'Callout (danger)', icon: 'i-lucide-octagon-alert', description: 'Red danger box' },
