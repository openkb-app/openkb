 ...
 * insert (editor/menu-items.ts). Each records the words its `[[` replaced,
 * keyed on the position it landed at, and the picker reads the record back
 * when a target is chosen (editor/components/DocLinkMenu.vue).
 *
...
let consumed: { text: string, at: number } | null = null

/** Records the words a `[[` at `at` replaced. */
export function recordLinkSelection(text: string, at: number): void {
  consumed = text.trim() ? { text, at } : null
}

/** The words the `[[` at `at` replaced. Reads once. */
export function takeLinkSelection(at: number): string | null {
  const held = consumed
  consumed = null
  return held && held.at === at ? held.text : null
}
