...
const { chat, messages, status, needsAuth, error, clear, unsent: prompt, scrollTop, scope, askedScopes } = useChatPanelSession()
...
const suggestions = [
  ...
  'What changed in this space this week?',
]
...
/** The space the next question goes out under, or null for the whole base. */
function currentScopeName(): string | null {
  return scope.value === SCOPE_ALL ? null : scopeLabel(scope.value, spaces.value)
}
...
function send(text: string) {
  ...
  askedScopes.value.push(currentScopeName())
...
// What each answer was asked, and the scope it went out under, so the reader
// is told what was searched. The scope is the one the question was sent with,
// not the one the picker shows now: picking another space later does not
// rewrite what an earlier answer searched.
const questions = computed(() => {
  const asked = new Map<string, { text: string, scopeName: string | null }>()
  let text = ''
  ...
  for (const m of messages.value) {
    if (m.role === 'user') {
      text = textFor(m)
      scopeName = askedScopes.value[sent++] ?? null
    }
    else asked.set(m.id, { text, scopeName })
  }
  return asked
})

function questionFor(m: UIMessage): string {
  return questions.value.get(m.id)?.text ?? ''
}

function scopeNameFor(m: UIMessage): string | null {
  return questions.value.get(m.id)?.scopeName ?? null
}
      ...
      >
        <template #content="{ message }">
          ...
          <ChatAnswerState
            ...
            :scope-name="scopeNameFor(message)"
          ...
          <!-- What the assistant did to answer, folded away under the answer. -->
          <ChatToolCalls v-if="message.role === 'assistant'" :runs="toolRunsFor(message)" />
        </template>
      </UChatMessages>
