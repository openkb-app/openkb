...
class GroundingSubscriber implements EventSubscriberInterface {
   ...
   *
   * @var array<string, array{question: string, context: array<string, mixed>, sources: \Drupal\ai_rag_cite\ValueObject\Source[]}>
   */
  private array $held = [];
  ...
  public function ground(PreGenerateResponseEvent $event): void {
    ...
    $held = $this->held[$settings->assistantId] ?? ['question' => '', 'context' => [], 'sources' => []];
    ...
    if ($question !== '' && ($question !== $held['question'] || $context !== $held['context'])) {
      try {
        $held = [
          'question' => $question,
          'context' => $context,
          'sources' => $this->evidence($question, $context, $settings),
        ];
      }
        ...
        $this->held[$settings->assistantId] = ['question' => $question, 'context' => $context, 'sources' => []];
    ...
    $input->setSystemPrompt(
      $this->grounding->systemPrompt($input->getSystemPrompt(), $held['sources'], $settings),
    );
  }
   ...
   *   The sources.
   */
  private function evidence(string $question, array $context, GroundingSettings $settings): array {
    $sources = $this->grounding->gate($this->retriever($settings)->retrieve($question, $context), $settings);
