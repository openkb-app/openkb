...
describe('the characters a space avatar shows', () => {
  it('takes the initials of a multi-word name', () => {
    expect(spaceInitials('Team Wiki')).toBe('TW')
    expect(spaceInitials('Product & Design')).toBe('PD')
  })

  it('takes the first two letters of a one-word name', () => {
    expect(spaceInitials('Engineering')).toBe('EN')
    expect(spaceInitials('X')).toBe('X')
  })

  it('answers a question mark for a name with no word to take a letter from', () => {
