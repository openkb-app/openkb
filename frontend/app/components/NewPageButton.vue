 ...
 * handle answers to.
 *
 * The author types a title and nothing else. The server creates the page and
 * pathauto mints its `<space-slug>/<title>` alias, seeds a `# <title>` body and
 * lands it as a draft; this component then navigates to the new page in edit
 * mode, so creating and writing are one continuous move instead of a form
 * submission followed by a hunt for the Edit button.
 */
...
async function create() {
  ...
  try {
    const page = await $fetch<{ path: string, uuid: string }>('/api/kb', {
      method: 'POST',
      body: { title: title.value.trim(), space: targetSpace.value },
    })
