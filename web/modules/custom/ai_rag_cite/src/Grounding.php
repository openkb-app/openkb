...
class Grounding {
   ...
   * nothing, so the reader is shown where it came from instead. `%1$s` is the
   * scope the turn searched, which the answer names too: one space came back
   * empty where another would have answered.
   */
  public const NO_SOURCES_INSTRUCTION = 'Nothing in %1$s was found for this question. If it asks about the knowledge base\'s content, say so plainly, name %1$s as what was searched, and stop; otherwise answer briefly, and cite nothing.';
   ...
   * that instead, together with the scope it searched; the extra guidance
   * still holds, the contract has nothing to hold it to.
   *
   ...
   *   The assistant's grounding settings.
   * @param string $scope
   *   What the turn searched, as the retriever words it.
   *
   * @return string
   *   The system prompt to run with.
   */
  public function systemPrompt(string $systemPrompt, array $sources, GroundingSettings $settings, string $scope): string {
    if (!$sources) {
      return implode("\n\n", array_filter([
        trim($systemPrompt),
        trim($settings->extraGuidance),
        sprintf(self::NO_SOURCES_INSTRUCTION, $scope),
      ]));
    }
